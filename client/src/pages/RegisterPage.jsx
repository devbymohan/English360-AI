...
export const RegisterPage = () => {
  ...
  const { register, googleLogin, logout, user, redirectError, clearRedirectError } = useAuth();
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (res?.redirecting) {
        // Mobile redirect fallback in progress
        return;
      }
      if (res?.user) {
        const target = res.user.assessmentCompleted ? '/dashboard' : '/assessment';
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-3 sm:p-6 md:p-8">
      <div className="max-w-4xl w-full bg-white rounded-3xl border border-slate-100 shadow-elevated overflow-hidden grid grid-cols-1 md:grid-cols-2">
        {/* Left Marketing Panel (Visible on Desktop / Tablets) */}
        ...
        <div className="p-6 sm:p-8 md:p-10 flex flex-col justify-center">
          ...
          <div className="hidden md:flex justify-end text-xs text-slate-500 mb-4">
          ...
          {/* Active session banner if user is already logged in on this device */}
          {user && (
            <div className="mb-4 p-3.5 rounded-2xl bg-amber-50 border border-amber-200 text-xs text-amber-900 flex flex-col gap-2.5 animate-in fade-in">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex-1 py-1.5 px-2.5 bg-white border border-amber-300 rounded-xl font-bold text-amber-900 hover:bg-amber-100 transition text-center shadow-xs flex items-center justify-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="flex-1 py-1.5 px-2.5 bg-brand-600 rounded-xl font-bold text-white hover:bg-brand-700 transition text-center shadow-xs flex items-center justify-center gap-1.5"
                >
                  <span>Go to App</span>
          ...
          {/* Social Sign-In (Top for fast mobile tap) */}
          <button
            type="button"
            onClick={handleGoogleSignIn}
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border border-slate-200 text-xs font-bold text-slate-700 hover:bg-slate-50 active:bg-slate-100 transition disabled:opacity-60 shadow-xs cursor-pointer"
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            <Button
              type="submit"
              size="lg"
              className="w-full rounded-xl mt-3 py-3"
              loading={loading}
              disabled={loading}
            >
          ...
          {/* Bottom helper on mobile */}
          <div className="mt-5 text-center text-xs text-slate-500 md:hidden">
